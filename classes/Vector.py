import math
import typing
import numpy as np

# Typing
Vector = typing.NewType("Vector", object)

class Vector(object):
    def __init__(self, x: int, y: int) -> None:
        self.x = x
        self.y = y
    

    ### Normal Algebraic func
    def __add__(self, other: Vector) -> Vector:
        """Add two vectors together using the '+' operator

            ### Args:
                other: A vector.
        """
        if(type(other) == Vector):
            return Vector(self.x + other.x, self.y + other.y)


    def __sub__(self, other: Vector) -> Vector:
        """Subtract two vectors using the '-' operator

            ### Args:
                other: A vector.
        """
        if(type(other) == Vector):
            return Vector(self.x - other.x, self.y - other.y)


    def __mul__(self, other: float) -> Vector:
        """Multiply a vector using the '*' operator

            ### Args:
                other: A floating point.
        """
        if(isinstance(other, (int, float)) and not isinstance(other, bool)): #Check if other is a number
            return Vector(self.x * other, self.y * other);


    def __div__(self, other:int) -> Vector:
        """Divide a vector using the '/' operator

            ### Args:
                other: A floating point.
        """
        if(isinstance(other, (int, float)) and not isinstance(other, bool)): #Check if other is a number
            return Vector(self.x * 1/other, self.y * 1/other);

    def __matmul__(self, other: Vector) -> Vector:
        """Multiply the two vectors using the dot product method via python's '@' operator

            ### Explaination:
            Python 3.5 introduces the '@' operator, mainly for matrix multlipication.
            We use this feature to make our code pretier and more readable.

            ### Args:
                other: A vector.
        """
        if(type(other) == Vector):
            return self.x*other.x + self.y*other.y
        else:
            raise TypeError(f"Type of other vector is not valid: '{type(other).__name__}' is not a valid Vector");  
        
    ### Self Algebraic func
    def __iadd__(self, other: Vector) -> None:
        self.x += other.x
        self.y += other.y
        return self
    
    def __isub__(self, other: Vector) -> None:
        self.x -= other.x
        self.y -= other.y
        return self

    def __str__(self):
        return f"({self.x:.2f}, {self.y:.2f})"
    
    def __repr__(self):
        return f"Vector({self.x}, {self.y})"

    def __abs__(self):
        return math.sqrt(self.x**2 + self.y**2)
    
    def dot(self, other: Vector):
        if(type(other) == Vector):
            return self.x*other.x + self.y*other.y
        else:
            raise TypeError(f"Type of other vector is not valid: '{type(other).__name__}' is not a valid Vector");

    def rotation(self, other=None):
        if(type(other) == Vector):
            return math.acos((self @ other)/abs(self) * abs(other))
        elif(other == None):
            return math.acos(self.x /abs(self))

def funcname(self, parameter_list) -> Vector:
    pass

### Testing###
if __name__ == "__main__":
    v1 = Vector(1,0)
    print(v1.rotation(),math.pi/4)

    v2 = Vector(1,-1)
    print(v2.rotation())

    v3 = Vector(2,2)
    v3 += v1;

    print(v3 * 2.0)
    print(v1.dot(v2))
    print(v1.dot(v1))
    print(v1 + v2)
    print(v2 - v1)
    print(abs(v1))
